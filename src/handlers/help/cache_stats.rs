...
fn generate_recommendations(stats: &crate::middleware::cache::CacheStats) -> Vec<String> {
    ...
    if total_items > 10000 {
        recommendations.push("⚠️ High number of cached items. Consider reducing TTL or max capacity to save memory.".to_string());
    }
    ...
    if let Some(&global_size) = stats.cache_sizes.get("global_stats") {
        if global_size == 0 && stats.total_requests > 50 {
            recommendations.push("💡 Global stats cache is empty. Consider pre-warming with /api/beatmap/count.".to_string());
        }
    }
