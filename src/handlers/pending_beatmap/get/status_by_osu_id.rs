...
pub async fn handler(
    ...
    let position: i64 = PendingBeatmap::position_by_osu_id(pool, id).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?.ok_or(StatusCode::NOT_FOUND)?;
    let total: i64 = PendingBeatmap::count(pool).await.map_err(|_| StatusCode::INTERNAL_SERVER_ERROR)?;
