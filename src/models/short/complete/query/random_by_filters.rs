...
pub async fn random_by_filters(
    ...
    let (query, param_count) = build_random_query(filters);
    let query_builder = bind_random_params(sqlx::query(&query), filters);
...
// -------------------------------------------
// Construire la requête SQL avec les filtres pour random
// -------------------------------------------
fn build_random_query(filters: &Filters) -> (String, usize) {
    ...
    // Utiliser ORDER BY RANDOM() pour un tri aléatoire
...
// -------------------------------------------
// Lier les paramètres à la requête SQL pour random
// -------------------------------------------
fn bind_random_params<'q>(mut query_builder: sqlx::query::Query<'q, sqlx::Postgres, sqlx::postgres::PgArguments>, filters: &Filters)
