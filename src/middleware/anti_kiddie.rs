...
fn contains_attack_patterns(request: &Request) -> bool {
    ...
    if let Some(query) = request.uri().query() {
        ...
        tracing::debug!("🔍 Analyzing query: '{}' (decoded: '{}')", query, decoded_query);
        
        for pattern in ATTACK_PATTERNS {
            let pattern_lower = pattern.to_lowercase();
            if query_lower.contains(&pattern_lower) {
                tracing::warn!("🚨 Attack pattern detected: '{}' in query: '{}'", pattern, decoded_query);
                return true;
            }
        }
        ...
        if query_lower.contains("'") && (
            ...
            tracing::warn!("🚨 SQL injection pattern detected in query: '{}'", decoded_query);
            return true;
        }
    }
...
pub async fn cleanup_old_entries() {
    ...
    loop {
        ...
        info!("🧹 Cleaned up attack tracker. Active entries: {}", tracker.len());
    }
}
