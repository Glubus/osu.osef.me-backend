...
impl CacheStore {
    ...
    pub fn get_stats(&self) -> CacheStats {
        ...
        stats.cache_sizes.insert("global_stats".to_string(), self.global_stats.entry_count());
        stats.cache_sizes.insert("filtered_queries".to_string(), self.filtered_queries.entry_count());
        stats.cache_sizes.insert("individual_beatmaps".to_string(), self.individual_beatmaps.entry_count());
        stats.cache_sizes.insert("pending_status".to_string(), self.pending_status.entry_count());
    ...
    pub async fn invalidate_related_caches(&self, path: &str) {
        if path.contains("/api/beatmap/") && !path.contains("/get/") {
            ...
            info!("🗑️ Invalidating beatmap-related caches due to modification at: {}", path);
...
async fn response_to_cached(response: Response) -> Result<CachedResponse, Box<dyn std::error::Error + Send + Sync>> {
...
fn cached_to_response(cached: CachedResponse) -> Response {
    ...
    *response.headers_mut() = cached.headers;
    ...
    response.headers_mut().insert("X-Cache-Status", "HIT".parse().unwrap());
    response.headers_mut().insert(
        "X-Cache-Age", 
        cached.created_at.elapsed().as_secs().to_string().parse().unwrap()
    );
        ...
        match response_to_cached(response).await {
            Ok(cached_response) => {
                ...
                tokio::spawn(async move {
                    CACHE_STORE.insert(cache_type, cache_key.clone(), cached_clone).await;
                ...
                response.headers_mut().insert("X-Cache-Status", "MISS".parse().unwrap());
                Ok(response)
            },
            Err(e) => {
