...
impl BeatmapProcessor {
    ...
    pub async fn add_checksums(&self, checksums: Vec<String>) -> Result<()> {
        ...
        for checksum in checksums_to_add {
            let _ = PendingBeatmap::insert(db_ref.get_pool(), &checksum).await?;
        }
    ...
    fn start_processing_thread(&self) {
        ...
        *processing = true;
        ...
        thread::spawn(move || {
            ...
            rt.block_on(async {
                Self::process_queue(is_processing, db).await;
            });
        });
    }
        ...
        loop {
            ...
            let maybe_pending = {
                ...
                if let Some(db_ref) = db_guard.as_ref() {
                    match PendingBeatmap::oldest(db_ref.get_pool()).await {
                        Ok(p) => p,
                        Err(e) => {
                            error!("Erreur lors de la récupération du plus ancien pending_beatmap: {}", e);
                            None
                        }
                    }
                } else {
            ...
            let pending = match maybe_pending {
                Some(p) => p,
                None => {
                    // Plus rien à traiter, arrêter le thread
                    let mut processing = is_processing.lock().unwrap();
                    *processing = false;
                    break;
                }
            ...
            // Traiter le checksum
            let result = Self::process_single_checksum(&db, pending.hash.clone()).await;

            // Supprimer de pending_beatmap (succès ou échec)
            {
                let db_guard = db.lock().unwrap();
                if let Some(db_ref) = db_guard.as_ref() {
            ...
            // Si échec, ajouter à failed_query
            if let Err(e) = result {
                let db_guard = db.lock().unwrap();
                if let Some(db_ref) = db_guard.as_ref() {
                    if let Err(e2) = FailedQuery::insert(db_ref.get_pool(), &pending.hash).await {
                        error!("Impossible d'insérer dans failed_query: {}", e2);
                    }
                }
                error!("Erreur lors du traitement du checksum {}: {}", pending.hash, e);
            } else {
                info!("Succès du traitement du checksum: {}", pending.hash);
            }
    ...
    async fn process_single_checksum(
        ...
        // Vérifier si déjà dans failed_query
        if FailedQuery::exists_by_hash(db_ref.get_pool(), &checksum).await? {
            return Err(anyhow::anyhow!("Checksum {} existe déjà dans failed_query", checksum));
        }
        ...
        if beatmap_extended.mode != GameMode::Mania {
            return Err(anyhow::anyhow!("Beatmap non mania: {}", beatmap_extended.mode));
        }

        if beatmap_extended.cs != 4.0 {
            return Err(anyhow::anyhow!("Beatmap non mania avec CS=4.0: {}", beatmap_extended.cs));
        }
        
        // Gérer le beatmapset
        let beatmapset_id = if let Some(mapset_extended) = beatmap_extended.mapset {
            let beatmapset = Beatmapset::from(*mapset_extended);
        ...
        // Récupérer et traiter le fichier osu!
            ...
            Ok(osu_file) => {
                ...
                unsafe {
                    if let Some(calc) = &CALC {
                        ...
                        let _ = msd.insert_into_db(db_ref.get_pool()).await?;
                    } else {
